...
class ChestXRay_CNN(nn.Module):
    def __init__(self, in_channels=3, input_norm=None, **kwargs):
        super(ChestXRay_CNN, self).__init__()
        self.in_channels = in_channels
        self.features = None
        self.classifier = None
        self.norm = None
    ...
    def build(self, input_norm=None, num_groups=None,
        ...
        if self.in_channels == 3:
            hidden = 128
            self.classifier = nn.Sequential(nn.Linear(c * 4 * 4, hidden), act(), nn.Linear(hidden, 2))
        else:
            self.classifier = nn.Linear(c * 8 * 8, 2)
...
class Retinal_CNN(nn.Module):
    def __init__(self, in_channels=3, input_norm=None, **kwargs):
        super(Retinal_CNN, self).__init__()
        self.in_channels = in_channels
        self.features = None
        self.classifier = None
        self.norm = None
    ...
    def build(self, input_norm=None, num_groups=None,
        ...
        if self.in_channels == 3:
            hidden = 128
            self.classifier = nn.Sequential(nn.Linear(c * 4 * 4, hidden), act(), nn.Linear(hidden, 4))
        else:
            self.classifier = nn.Linear(c * 8 * 8, 4)
